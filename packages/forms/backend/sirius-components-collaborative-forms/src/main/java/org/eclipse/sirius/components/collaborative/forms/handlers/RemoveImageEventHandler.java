...
import org.eclipse.sirius.components.collaborative.forms.dto.RemoveImageInput;
...
/**
 * The handler of the image picker remove image event.
 *
 * @author Jerome Gout
 */
@Service
public class RemoveImageEventHandler implements IFormEventHandler {
    ...
    public RemoveImageEventHandler(IFormQueryService formQueryService, ICollaborativeFormMessageService messageService, MeterRegistry meterRegistry) {
    ...
    public boolean canHandle(IFormInput formInput) {
        return formInput instanceof RemoveImageInput;
    }
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, Form form, IFormInput formInput) {
        ...
        String message = this.messageService.invalidInput(formInput.getClass().getSimpleName(), RemoveImageInput.class.getSimpleName());
        IPayload payload = new ErrorPayload(formInput.id(), message);
        ChangeDescription changeDescription = new ChangeDescription(ChangeKind.NOTHING, formInput.representationId(), formInput);

        if (formInput instanceof RemoveImageInput input) {
            ...
            if (optionalImagePicker.map(AbstractWidget::isReadOnly).filter(Boolean::booleanValue).isPresent()) {
                ...
                var handler = optionalImagePicker.filter(ImagePicker.class::isInstance)
                        ...
                        .map(ImagePicker::getRemoveValueHandler);
